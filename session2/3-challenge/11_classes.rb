...
class BeerSong
  # attr_accessor allows us to read and write to variable beers
  attr_accessor :beers
  # set the beers parameter between zero and 99  
  def initialize(beers)
    beers = 0  if beers < 0
    beers = 99 if beers > 99
    # sets beers on the instance method beers
    self.beers = beers
  end
  #.downto iterates from beers to 1 doing i each time
  def print_song
    beers.downto 1 do |i|
      print_stanza i
    end
