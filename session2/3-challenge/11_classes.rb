...
# Create the clasee called beer song
class BeerSong
  # This creates the setter and getter methods (def beers=, def beers)
  attr_accessor :beers

  def initialize(beers)
    # if beers is less that 0, beers = 0
    beers = 0  if beers < 0
    # if beer is > 99, beers = 99
    beers = 99 if beers > 99
    self.beers = beers
  end

  def print_song
    # for beers, from 99 down to 1, print_stanza for each iteration
    beers.downto 1 do |i|
      print_stanza i
    end
  end
  ...
  def print_stanza(n)
    # if n = 0 make a new string
    if n.zero?
      String.new
    else
      # substitues the number into the song and takes one away at the end
  ...
  # returns "bottle" or "bottles" depending if n = 1 or not
  def bottle(n)
