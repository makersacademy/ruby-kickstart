...
class User
	...
	def blogs
		Blog.all_blogs.select{ |blog| blog.user.object_id == self.object_id }.sort_by{ |blog| blog.date }.reverse
	end
...
class Blog
	...
	def ==(other)
		if other.class == Blog
			(self.date == other.date and self.user == other.user and self.text == other.text)
		end
	end
