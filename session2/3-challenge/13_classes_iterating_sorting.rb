...
def add_blog(d, t)
	new_blog = Blog.new(d, self, t)
	@list << new_blog
	new_blog
end

def blogs
	@list = list.sort_by {|blog| blog.date}.reverse
end
end
...
def summary
	text.split(" ").select.with_index {|word, ind| word if ind <= 9}.join(" ")
end
