...
class User
  ...
  def add_blog(date, text)
    ...
    @blogs<< new_blog
    new_blog
    #add blog with date and text
  end

  def blogs
    @blogs.sort_by{|blog| blog.date}.reverse
    #returns an array of all blogs by user
  end
end
...
class Blog
  ...
  def summary
    ...
    #return first 10 words from text
  end
  ...
  def ==(other)#how does this even work
    p self.class
    ...
    blog = user.add_blog(Date.parse("10-12-2005"), "other text")
    puts user.blogs.map{|blog,date| puts blog.date}
