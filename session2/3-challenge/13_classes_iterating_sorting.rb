...
class Blog
  ...
  def summary # returns the first 10 words from the text (or the entire text if it is less than 10 words)
    text.split[0..9].join(" ")
  end

  def entry #will return the data in the form of "user(its username var, not the object) date full text of blog entry"
    "#{user.username} #{date}\n#{text}"
  end
