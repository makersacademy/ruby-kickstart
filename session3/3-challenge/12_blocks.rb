...
def pay_by_visa(order, ccn)
  type = Proc.new {order.payment :type => :visa, :ccn => ccn
  order.verify_payment}
  pay_by(order, ccn, &type)
end

def pay_by_check(order)
  type = Proc.new {order.payment :type => :check, :signed => true}
  pay_by(order, &type)
end

def pay_by_cash(order)
  type = Proc.new{order.payment :type => :cash}
  pay_by(order, &type)
end

def pay_by_store_credit(order, current_user)
  type = Proc.new {order.payment :type => :store_credit
